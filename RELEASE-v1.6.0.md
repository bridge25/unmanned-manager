# v1.6.0 - JARVIS SDK (IPC v2.1)

> 🔌 **Major Release**: Worker가 PM에게 작업 상태를 실시간 전달하는 Python SDK!

---

## ⚠️ 사전 요구사항: 자신의 MindCollab 인스턴스

**JARVIS SDK를 사용하려면 먼저 자신의 MindCollab 인스턴스가 필요합니다!**

각 사용자는 독립적인 MindCollab 환경을 가집니다:
- 자신의 MindCollab 배포 (Railway 등)
- 자신의 API 엔드포인트 URL
- 자신의 API Key

```
Tony님 MindCollab ─────► Tony님 PM Claude
수강님 MindCollab ─────► 수강님 PM Claude  (각자 독립!)
대웅님 MindCollab ─────► 대웅님 PM Claude
```

**MindCollab 배포 가이드**: [mindcollab 저장소](https://github.com/bridge25/mindcollab) 참조

---

## 🔌 JARVIS SDK란?

**Claude Code Worker에서 작업 진행 상황을 PM(Project Manager)에게 자동으로 알려주는 통신 SDK입니다.**

```
작업 시작 → SDK가 PM에게 알림 → 작업 완료 → SDK가 PM에게 알림
                                 ↓
                         에러 발생 → SDK가 자동으로 블로커 알림
```

### 왜 필요한가?

기존 문제:
- Worker가 작업 완료해도 PM이 모름
- 에러 발생해도 PM이 한참 후에야 발견
- 수동으로 결과 파일 확인 필요

SDK 사용 후:
- ✅ 작업 시작/완료 **실시간 알림**
- ✅ 에러 발생 시 **자동 블로커 알림**
- ✅ PM이 팀 전체 진행 상황 **한눈에 파악**

---

## 📁 위치

[`packages/jarvis-sdk/`](./packages/jarvis-sdk/)

---

## 📚 문서

- **상세 가이드**: [`packages/jarvis-sdk/README.md`](./packages/jarvis-sdk/README.md)
- **API 레퍼런스**: 아래 사용법 섹션 참조

---

## 🎯 대상 사용자

- **수강** - 기획/리서치 작업 시 진행 상황 공유
- **대웅** - 개발 작업 시 완료/블로커 알림
- **모든 Worker Claude 사용자**

---

## 🆕 새로운 파일

| 파일 | 설명 |
|------|------|
| `packages/jarvis-sdk/` | SDK 메인 폴더 |
| `__init__.py` | 패키지 진입점 |
| `task.py` | `JarvisTask` 메인 클래스 |
| `client.py` | API 클라이언트 (재시도, outbox) |
| `config.py` | 설정 관리 |
| `pyproject.toml` | 패키지 설정 |
| `test_sdk.py` | 테스트 코드 |

---

## 🪟 Windows 설치 가이드

### 1단계: SDK 다운로드

```powershell
# 저장소 클론 (처음 한 번만)
cd $HOME\Desktop
git clone https://github.com/bridge25/unmanned-manager.git

# 또는 기존 저장소 업데이트
cd $HOME\Desktop\unmanned-manager
git pull origin main
```

### 2단계: SDK 복사

```powershell
# 자신의 프로젝트로 SDK 복사
Copy-Item -Path "packages\jarvis-sdk" -Destination "C:\Users\YourName\Desktop\your-project\jarvis_sdk" -Recurse

# 폴더 구조 확인
dir your-project\jarvis_sdk
# __init__.py, task.py, client.py, config.py 등이 보여야 함
```

### 3단계: 환경변수 설정

⚠️ **중요: 자신의 MindCollab 인스턴스 정보를 사용하세요!**

**PowerShell (임시 설정)**
```powershell
$env:JARVIS_API_KEY = "<자신의_MindCollab_API_키>"
$env:JARVIS_API_URL = "<자신의_MindCollab_URL>/api"  # 예: https://your-mindcollab.up.railway.app/api
$env:JARVIS_WORKER_ID = "sookang"  # 본인 이름으로 변경!
```

**영구 설정 (시스템 환경변수)**
1. Windows 검색 → "환경 변수" → "시스템 환경 변수 편집"
2. "환경 변수" 버튼 클릭
3. "사용자 변수"에서 "새로 만들기":
   - 변수 이름: `JARVIS_API_KEY`
   - 변수 값: `<자신의_MindCollab_API_키>`
4. 같은 방식으로 추가:
   - `JARVIS_API_URL` = `<자신의_MindCollab_URL>/api`
   - `JARVIS_WORKER_ID` = `본인이름`

### 4단계: 테스트

```powershell
cd your-project
python -c "from jarvis_sdk import JarvisTask; print('SDK 로드 성공!')"
```

---

## 🍎 macOS 설치 가이드

```bash
# 1. SDK 복사
cp -r unmanned-manager/packages/jarvis-sdk ~/Desktop/your-project/jarvis_sdk

# 2. 환경변수 설정 (~/.zshrc 또는 ~/.bashrc에 추가)
# ⚠️ 자신의 MindCollab 인스턴스 정보로 변경!
echo 'export JARVIS_API_KEY="<자신의_MindCollab_API_키>"' >> ~/.zshrc
echo 'export JARVIS_API_URL="<자신의_MindCollab_URL>/api"' >> ~/.zshrc
echo 'export JARVIS_WORKER_ID="본인이름"' >> ~/.zshrc
source ~/.zshrc

# 3. 테스트
python3 -c "from jarvis_sdk import JarvisTask; print('SDK 로드 성공!')"
```

---

## ⚡ 사용법

### 기본 사용 (권장)

```python
from jarvis_sdk import JarvisTask

# Context Manager 사용 - 에러 시 자동 블로커 알림
with JarvisTask(
    task_id="data_processing_001",   # 작업 ID (고유해야 함)
    node_id="N148",                   # MindCollab 노드 ID (선택)
    worker_id="sookang"               # 기본값: 환경변수 JARVIS_WORKER_ID
) as task:

    # 1. 작업 시작 알림 (PM에게 "수강이 작업 시작함" 알림)
    task.start("고객 데이터 분석 시작")

    # 2. 중간 로그 (선택사항)
    task.log("1000건 중 500건 처리 완료", level="info")

    # 3. 작업 완료 (PM에게 "수강이 작업 완료함" 알림)
    task.complete(
        result={"processed": 1000, "errors": 0},
        summary="고객 데이터 1000건 분석 완료"
    )

# ⚠️ with 블록 안에서 에러 발생 시:
# → SDK가 자동으로 task.blocked() 호출
# → PM에게 "수강이 블로커 만남" 알림
```

### 이벤트 타입

| 메서드 | 이벤트 | PM에게 보이는 알림 |
|--------|--------|-------------------|
| `task.start("설명")` | task_started | "🚀 [sookang] 작업 시작: 설명" |
| `task.complete(result, summary)` | task_completed | "✅ [sookang] 작업 완료: summary" |
| `task.blocked(reason)` | task_blocked | "🚫 [sookang] 블로커: reason" |
| `task.log("메시지")` | task_log | (warning/error만 알림) |

### 로그 레벨

```python
task.log("일반 정보", level="info")      # 알림 안 감 (로그만)
task.log("주의 필요", level="warning")   # PM에게 알림
task.log("에러 발생", level="error")     # PM에게 알림
```

### 블로커 타입

```python
task.blocked(
    reason="외부 API 응답 없음",
    blocker_type="external"    # error, dependency, input_needed, external
)
```

| blocker_type | 의미 |
|--------------|------|
| `error` | 코드 에러, 예외 발생 |
| `dependency` | 다른 작업 완료 대기 |
| `input_needed` | 사용자 입력 필요 |
| `external` | 외부 시스템 문제 (API, DB 등) |

---

## 🔄 Fail-open 동작

**API 호출 실패해도 작업은 계속됩니다!**

```
API 호출 → 실패 → .jarvis/outbox/에 저장 → 작업 계속 진행
                        ↓
              나중에 재전송 가능
```

```python
# 기본 동작: fail-open (API 실패해도 예외 안 남)
task = JarvisTask("my_task")

# strict 모드: API 실패 시 예외 발생
task = JarvisTask("my_task", strict=True)
```

---

## 🏗️ 아키텍처

```
┌─────────────────────────────────────────────────────────────┐
│                     Worker Claude                            │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  with JarvisTask(...) as task:                      │    │
│  │      task.start("작업 시작")                         │    │
│  │      task.complete(result)                          │    │
│  └──────────────────────┬──────────────────────────────┘    │
└─────────────────────────┼───────────────────────────────────┘
                          │
                          ▼
              ┌───────────────────────┐
              │    JARVIS SDK         │
              │  (이 패키지)           │
              └───────────┬───────────┘
                          │ HTTP POST
                          ▼
              ┌───────────────────────┐
              │   MindCollab API      │
              │  /api/jarvis/events   │
              └───────────┬───────────┘
                          │
                          ▼
              ┌───────────────────────┐
              │      Supabase         │
              │   jarvis_events 테이블 │
              └───────────┬───────────┘
                          │ Realtime
                          ▼
              ┌───────────────────────┐
              │    Push Daemon        │
              └───────────┬───────────┘
                          │
            ┌─────────────┴─────────────┐
            ▼                           ▼
    ┌───────────────┐          ┌───────────────┐
    │  Tony (사람)   │          │  PM Claude    │
    │ display-message│          │  inbox.jsonl  │
    │   (즉시 팝업)   │          │  (Pull 방식)   │
    └───────────────┘          └───────────────┘
```

---

## ❓ FAQ

### Q: 환경변수를 매번 설정해야 하나요?

A: 아니요! 시스템 환경변수에 한 번 설정하면 영구 적용됩니다.
   - Windows: 시스템 환경 변수 편집
   - macOS: `~/.zshrc`에 export 추가

### Q: SDK가 없는 프로젝트에서도 사용 가능한가요?

A: 네! `jarvis_sdk` 폴더를 프로젝트에 복사하면 바로 사용 가능합니다.
   외부 패키지 설치 필요 없음 (Zero Dependencies).

### Q: API 서버가 다운되면 어떻게 되나요?

A: 기본 fail-open 모드에서는 `.jarvis/outbox/`에 이벤트가 저장되고,
   작업은 정상 진행됩니다. 나중에 서버 복구 시 재전송 가능.

### Q: node_id는 뭔가요?

A: MindCollab의 프로젝트 노드 ID입니다. 선택사항이며,
   지정하면 해당 노드에 작업이 연결됩니다.
   예: `mc tasks` 명령으로 노드 ID 확인 가능.

---

## 🔑 환경변수 설정 예시

각자 **자신의 MindCollab 인스턴스** 정보를 사용합니다:

| 환경변수 | 값 |
|---------|-----|
| `JARVIS_API_KEY` | 자신의 MindCollab에서 설정한 API 키 |
| `JARVIS_API_URL` | 자신의 MindCollab URL + `/api` |
| `JARVIS_WORKER_ID` | 본인 이름 (예: `sookang`, `daewoong`) |

**예시 (수강님의 경우):**
```
JARVIS_API_KEY = sookang-jarvis-key-2026
JARVIS_API_URL = https://sookang-mindcollab.up.railway.app/api
JARVIS_WORKER_ID = sookang
```

⚠️ **MindCollab 인스턴스가 없으면 먼저 배포하세요!**
→ [mindcollab 저장소](https://github.com/bridge25/mindcollab) 참조

---

## 🆘 문제 해결

### "ModuleNotFoundError: No module named 'jarvis_sdk'"

```powershell
# jarvis_sdk 폴더가 프로젝트 루트에 있는지 확인
dir jarvis_sdk
# __init__.py 파일이 있어야 함
```

### "JARVIS_API_KEY environment variable is required"

```powershell
# 환경변수 확인
echo $env:JARVIS_API_KEY
# 비어있으면 설정 필요
```

### API 연결 실패

1. 인터넷 연결 확인
2. API URL 확인: `https://mindcollab-web-production.up.railway.app/api`
3. fail-open 모드에서는 `.jarvis/outbox/`에 저장됨 (나중에 재전송)

---

## 📞 지원

문제 발생 시:
1. 이 문서의 FAQ 확인
2. Tony님께 문의 (MindCollab 댓글 또는 직접 연락)

---

**Happy Coding! 🎉**
